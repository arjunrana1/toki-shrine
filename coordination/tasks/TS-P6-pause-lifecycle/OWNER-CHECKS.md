...
- **P6-O9:** Pass. **Improvement note (owner):** needs a way to dismiss the bubble — suggested drag to the bottom screen edge and discard, per common floating-button convention. Not implemented; changes PRD §6 screen 20 (bubble persists while a pause exists), so it awaits an owner product/PRD decision.
